...
async function loadFollowing() {
    try {
        ...
        followingIds = new Set(
            data.following.map((user) => String(user._id || user.id))
        );
    }
    catch (err) {
...
function renderUsers(users) {
    ...
    users.forEach((user) => {
        ...
        row.innerHTML = `
      <a href="/user-profile.html?id=${userId}" class="user-main user-profile-link">
        ${avatar}

        <div class="user-info">
          <strong class="user-name">${user.username}</strong>
          <span class="user-email">${user.email}</span>
          <span class="user-genres">${genres}</span>
        </div>
      </a>

      <button
        type="button"
        class="${isFollowing ? 'secondary-btn' : 'primary-btn'} btn-small follow-btn"
        data-user-id="${userId}"
...
usersList.addEventListener('click', async (event) => {
    ...
    try {
        ...
        if (!response.ok) {
            usersMessage.textContent = data.error || 'Action failed';
            return;
        }
        ...
        usersMessage.textContent = 'Action failed';
    }
});
...
async function loadAllUsers() {
    try {
        ...
        if (!response.ok) {
            usersMessage.textContent = data.error || 'Failed to load users';
            return;
        }
        ...
        usersMessage.textContent = 'Failed to load users';
    }
}
...
searchUsersForm.addEventListener('submit', async (event) => {
    ...
    try {
        ...
        if (!response.ok) {
            usersMessage.textContent = data.error || 'Search failed';
            return;
        }
        ...
        usersMessage.textContent = 'Search failed';
    }
});
...
editProfileForm.addEventListener('submit', async (event) => {
    ...
    const favoriteGenres = profileGenres.value
        .split(',')
    ...
    try {
        ...
        profileMessage.textContent = 'Update failed';
    }
});
...
deleteAccountBtn.addEventListener('click', async () => {
    ...
    try {
        ...
        if (!response.ok) {
            profileMessage.textContent = data.error || 'Delete failed';
            return;
        }
        ...
        profileMessage.textContent = 'Delete failed';
    }
});
