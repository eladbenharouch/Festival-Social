  ...
  try
  {
    const currentUser =
      await User.findById(
        ...
        'groups following favoriteGenres'
      );
    ...
    /*
      Demo users are created by seed.js.

      Their posts are always available
      as recommendations so a brand-new
      user immediately gets a populated
      personalized feed.
    */
    const demoUsers =
      await User.find(
    ...
    /*
      Keep the normal social-network logic:
      - own posts
      - posts from joined groups
      - posts from followed users

      Then also include seeded demo posts
      for recommendations/discovery.
    */
    const posts =
      await populatePost(
        Post.find(
    ...
    {
      /*
        Keep "posts" for compatibility
        with the current frontend until
        home.js is upgraded.
      */
      posts,
...
async function getPostsByGroup(req, res)
{
  ...
  catch (err)
  {
    ...
    {
      error:
        'Server error while fetching group posts'
    });
  }
}
...
async function searchPosts(req, res)
{
  ...
  catch (err)
  {
    ...
    {
      error:
        'Server error while searching posts'
    });
  }
}
